...
// ExportDetailedHotspots retrieves detailed information for each hotspot and writes to file
func (c *SonarClient) ExportDetailedHotspots(hotspots []Hotspot, filepath string) error {
	file, err := os.Create(filepath)
	if err != nil {
		return fmt.Errorf("error creating CSV file: %w", err)
	}

	fmt.Fprintln(file, "<html><head></head><body>")
	defer func() {
	...
	for i, h := range hotspots {
		...
		if i > 0 {
			...
			_, _ = fmt.Fprintln(file, "<hr>")
		}

		_, err = fmt.Fprintf(file, "<h1>%s / %s / %s</h1>\n<p>%s\n<p>%s:%d\n<p>%s\n<p>%s<p><p>",
			detail.Rule.VulnerabilityProbability,
			detail.Rule.SecurityCategory,
			detail.Rule.Name,
			detail.Message,
			detail.Component.LongName,
			detail.TextRange.StartLine,
			detail.Rule.VulnerabilityDescription,
			detail.Rule.RiskDescription,
		)

		// Flush after each write to ensure data is written in case of errors
		_ = file.Sync()
	}
